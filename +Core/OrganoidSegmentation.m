classdef OrganoidSegmentation < handle
    ...
    methods
        function obj = OrganoidSegmentation(raw,info)
        ...
        function gAdapt = segmentNuclei(obj)
           data = obj.getChannel('nucleus');
           fr = 49;
           %data = data1(:,:,49);
           %pre processing test
%            gData = imgaussfilt3(data,3);
%            
%            stdData = stdfilt(data);          
           medData = medfilt3(data);
           
%            gradData = imgradient3(data);
           
%            [~,gAdapt1] = imSegmentation.segmentStack(uint16(gData),'threshold',0.2,...
%                 'connectivity',27,'diskDim',2);
%            [~,gAdapt2] = imSegmentation.segmentStack(uint16(gradData),'threshold',0.2,...
%                 'connectivity',27,'diskDim',2);
           [~,gAdapt] = imSegmentation.segmentStack(uint16(medData),'threshold',0.5,...
                'connectivity',9,'diskDim',2);
%            [~,gAdapt2] = imSegmentation.segmentStack(uint16(medData(:,:,fr)),'threshold',0.2,...
%                 'connectivity',27,'diskDim',2);   
           figure
%            subplot(1,2,1)
           imagesc(gAdapt(:,:,fr))
...
%            axis image
%             
           contour = obj.getCellContour(gAdapt);
           ...
           obj.results.cellContour{fr} = contour;
          
           obj.plotCellContour(fr)
           
           
           obj.results.cellMask = logical(gAdapt);
        end
       

        function bw = segmentNucleiWatershed(obj)
            ...
            DataCorr = zeros(size(data,1), size(data,2), size(data,3)-Idx);
            for i = 1:size(DataCorr, 3)
                DataCorr(:,:,i) = data(:,:,i).*MeanIntInv(i,1);
            end

            DataCorr2 = DataCorr;
            DataCorr2(DataCorr2 > 1) = 1;

            DataCorr3 = medfilt3(DataCorr2);
            DataCorrInv = ~DataCorr3;

            %% Marker controlled watershed transformation
            DataCorrCells = DataCorrInv;
            DataCorrCellsLabelled = DataCorrCells;
            for i = 1:size(DataCorrCells, 3)
                DataCorrCells(:,:,i) = bwareafilt(DataCorrCells(:,:,i), [150 25000]);
            end
            DataCorrCellsLabelled = bwlabeln(DataCorrCells,6);
            
            se = strel(ones(6,6,3));
            %MaxLabel = imclose(DataCorrCellsLabelled,se);
            %MaxLabel2 = imerode(MaxLabel,se);
            MaxLabel2 = imerode(DataCorrCellsLabelled,se);
            MaxLabel3 = MaxLabel2;
            MaxLabel3(MaxLabel3 > 1) = 1;
            MaxLabel3 = bwareaopen(MaxLabel3, 400);
            
            DMap = bwdist(MaxLabel3);
            DMapList = DMap(:);
            DMapList(DMap == 0) = [];
            avDist = mean(DMapList);
            DMap2 = DMap;
            DMap2(DMap2 > avDist) = 0;
            
            DMap3 = zeros(size(DMap,1), size(DMap,2), size(DMap,3));
            DMap3(DMap2 == 0) = 1;
            DMap4 = bwdist(DMap3);
            ws = watershed(DMap4);

            i = 20;
            Test = labeloverlay(data(:,:,i), ws(:,:,i));
            figure()
            imshow(Test)

            SegmentsInt = zeros(size(ws));
            CC = bwconncomp(ws);
            for i = 1:CC.NumObjects
                SegmentList = CC.PixelIdxList{1,i};
                DMask = zeros(size(ws, 1), size(ws, 2), size(ws, 3));
                DMask(SegmentList) = 1;
                IntSegment = DataCorr;
                IntSegment(DMask == 0) = NaN;
                
                avInt = nanmean(IntSegment(:));
                SegmentsInt(DMask == 1) = avInt;
            end

            ReSegmentsInt = rescale(SegmentsInt);
            [Hist(:,1), Hist(:,2)] = imhist(ReSegmentsInt);
            Hist(Hist(:,1) == 0, :) = [];
            Hist(1,:) = [];
            MinHist = islocalmin(Hist(:,1));
            Idx = find(MinHist,1);
            Thr = Hist(Idx + 1, 2);
            ReSegmentsIntBg = ReSegmentsInt;
            ReSegmentsIntBg(ReSegmentsIntBg < Thr) = 0;
            
            CC = bwconncomp(ReSegmentsIntBg);
            stats = regionprops(CC, 'Area');
            AreaList = cell2mat(struct2cell(stats));
            [AreaBg, IdxBg] = max(AreaList); 

            AreaList(AreaList < 150000) = 0;
            AreaList(AreaList == AreaBg) = 1;
            
            SegmentedAreaCells = zeros(size(ws));
            for i = 1:size(CC.PixelIdxList, 2)
                SegmentedAreaCells(CC.PixelIdxList{1,i}) = AreaList(1,i);
            end
            
            SegmentedAreaCells(SegmentedAreaCells > 1) = 1;

            DMap = bwdist(SegmentedAreaCells);
            ws = watershed(DMap);
            
            [~,bw] = imSegmentation.segmentStack(uint16(DataCorr),'threshold',0.5,...
                'connectivity',27,'diskDim',2);
            figure
            imagesc(bw(:,:,fr))
            axis image
            ...
            DMap = DistMap.bwdistsc(~bw,weights);
            
            r = 3;
            Ds = imgaussfilt3(DMap, [2 2 1]);
            imD = -Ds;
