...
MainFolder = {'E:\Steven'};
DimensionFolders = {'2D'};
HourFolders = {'48hour', '24hour'};
CellineFolders = {'MCF7', 'KM12C', 'HeLa', 'A549'};
                ...
                for i = 1:size(CurrentFolder,1)
                    if isDirColumn(i,1) == 1
                        try
                            ...
                            for j = 1:size(SubFolder,1)
                                try
                                    if isSubDirColumn(j,1) == 1
                                        ...
                                        %% Particle Intensity
                                        [Int, MembraneInt, IntRatio, MembrIntRatio, Volume]= stack.getIntensityInside();
                                        CellInt = [CellInt; Int];
                                        MembrInt = [MembrInt; MembraneInt];
                                        CellDens = [CellDens; IntRatio];
                            ...
                            filenameCellDens = append(SubFolder(1).folder, filesep,'CellDens.mat');
                            filenameMembrDens = append(SubFolder(1).folder, filesep,'MembrDens.mat');
                            filenameVolume = append(SubFolder(1).folder, filesep,'VolumeList.mat');
                            save(filenameCell, 'CellInt');
                            save(filenameMembr, 'MembrInt');
                            save(filenameCellDens, 'CellDens');
